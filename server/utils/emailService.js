      ...
      const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
        ...
        body: JSON.stringify({
          ...
          template_params: {
            ...
            'From Name': 'SkillBridge',
            fromName: 'SkillBridge',
            company_name: 'SkillBridge',
            'Company Name': 'SkillBridge',
...
// ─── 6. Site Visitor / Access Tracker Alert ──────────────
async function sendVisitorAlertEmail(logData) {
  const adminEmail = process.env.EMAIL_USER || 'drsvvn25@gmail.com';
  const now = new Date();
  const timeStr = now.toLocaleString('en-IN', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });

  const html = baseTemplate(`
    <h2 style="margin:0 0 8px;font-size:1.4rem;font-weight:700;color:#f1f5f9;">👁️ New Website Visit Tracker Alert</h2>
    <p style="margin:0 0 20px;color:#94a3b8;font-size:0.95rem;line-height:1.6;">
      Someone accessed your SkillBridge site hosted on Render. Here are the visit tracker details:
    </p>
    <div style="background:#0f172a;border-radius:12px;padding:24px;margin-bottom:24px;border:1px solid #1dbf73;">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="padding:8px 0;color:#64748b;font-size:0.88rem;font-weight:600;">👤 Author / User Name</td>
          <td style="padding:8px 0;color:#1dbf73;font-size:0.95rem;text-align:right;font-weight:700;">${logData.user_name || 'Divy / Guest'}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#64748b;font-size:0.88rem;font-weight:600;">📍 IP Address</td>
          <td style="padding:8px 0;color:#0ea5e9;font-size:0.95rem;text-align:right;font-weight:700;font-family:'Courier New',monospace;">${logData.ip}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#64748b;font-size:0.88rem;font-weight:600;">🌐 Visited Page</td>
          <td style="padding:8px 0;color:#e2e8f0;font-size:0.88rem;text-align:right;font-family:'Courier New',monospace;">${logData.page}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#64748b;font-size:0.88rem;font-weight:600;">⏰ Timestamp</td>
          <td style="padding:8px 0;color:#f59e0b;font-size:0.88rem;text-align:right;">${timeStr}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#64748b;font-size:0.88rem;font-weight:600;">💻 Device / Browser</td>
          <td style="padding:8px 0;color:#94a3b8;font-size:0.78rem;text-align:right;word-break:break-all;">${logData.user_agent || 'Unknown'}</td>
        </tr>
      </table>
    </div>
    <p style="margin:0;font-size:0.8rem;color:#64748b;text-align:center;">
      This automated alert was generated by SkillBridge Visitor Tracking Engine.
    </p>
  `);

  await sendMail(adminEmail, `👁️ Site Visit: ${logData.user_name} (${logData.ip})`, html, {
    fullName: logData.user_name,
    company_name: 'SkillBridge'
  });
}
